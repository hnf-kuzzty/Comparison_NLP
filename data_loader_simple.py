...
class DataLoader:
    ...
    def load_steam_games_data(self):
        """Load Steam games data from local files"""
        try:
            ...
            for file_type, file_path in local_files.items():
                if os.path.exists(file_path):
                    ...
                    try:
                        ...
                        print(f"  ❌ Error loading {file_type}.csv: {e}")
                else:
                    print(f"  ⚠️  {file_path} not found")
            
            if not loaded_data:
                print("  📥 No local files found, creating sample gaming data...")
                return self.create_sample_gaming_data()
            
            # Process the loaded data
            if 'games' in loaded_data:
                self.steam_data = loaded_data['games'].copy()
                
                # If we have recommendations, try to merge
                if 'recommendations' in loaded_data:
                    print("  🔗 Merging with recommendations...")
                    rec_df = loaded_data['recommendations']
                    
                    # Find common column for merging
                    games_cols = set(self.steam_data.columns)
                    rec_cols = set(rec_df.columns)
                    ...
                    if common_cols:
                        ...
                        # Simple merge - just take first recommendation per game
                        rec_simple = rec_df.groupby(merge_col).first().reset_index()
                        self.steam_data = self.steam_data.merge(rec_simple, on=merge_col, how='left', suffixes=('', '_rec'))
                ...
                print("  ❌ No games.csv found, creating sample data...")
                return self.create_sample_gaming_data()
